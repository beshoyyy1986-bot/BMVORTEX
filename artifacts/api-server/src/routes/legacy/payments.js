  ...
  if (isCard) {
    ...
    if (cid && !cards.find(c => c.id === cid)) {
      let n = obj.card_association_name
        ...
        : obj.card_brand
          ? `${obj.card_brand} •••• ${obj.last_four_digits}`
          : `Card •••• ${obj.last_four_digits}`;
...
router.post('/fetch-cards', async (req, res) => {
  ...
  try {
    ...
    const apiRes = await fetch(`${auth.origin}/api/graphql/`, {
      method: 'POST',
    ...
    res.json({ ok: true, cards, userId: auth.userId });
  } catch (e) {
    console.error('[payments/fetch-cards]', e.message);
...
router.post('/remove-card', async (req, res) => {
  ...
  try {
    ...
    console.error('[payments/remove-card]', e.message);
    res.json({ ok: false, reason: e.message });
  }
});
...
router.post('/add-funds', async (req, res) => {
  ...
  try {
    ...
    console.error('[payments/add-funds]', e.message);
    res.json({ ok: false, reason: e.message });
  }
});
...
router.post('/set-primary', async (req, res) => {
  ...
  try {
    ...
    console.error('[payments/set-primary]', e.message);
    res.json({ ok: false, reason: e.message });
  }
});
...
router.post('/switch-old', async (req, res) => {
  ...
  try {
    ...
    const apiRes = await fetch(`${auth.origin}/api/graphql/?_callFlowletID=10709&_triggerFlowletID=10704`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', cookie: cookieStr, 'User-Agent': 'Mozilla/5.0' },
      body,
    });
    ...
    console.error('[payments/switch-old]', e.message);
    res.json({ ok: false, reason: e.message });
  }
});
