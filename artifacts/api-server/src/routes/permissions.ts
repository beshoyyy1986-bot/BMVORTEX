...
router.post("/grant-permissions", async (req, res): Promise<void> => {
  ...
  const actorIdStr: string = String(session.userId || "");
  ...
  try {
    const response = await fetch(`${origin}/api/graphql/`, {
      ...
      headers: {
        ...
        "x-fb-lsd": lsd,
        "x-fb-friendly-name": "BusinessAccountPermissionTasksForUserModalMutation",
    ...
    if (!response.ok) {
      res.status(400).json({
        error: metaError || `Meta API returned status ${response.status}`,
