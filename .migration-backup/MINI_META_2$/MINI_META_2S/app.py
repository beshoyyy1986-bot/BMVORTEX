...
app = FastAPI(title="MINI META 2$", version="2.0")
...
async def extract_fb_tokens(page) -> dict:
    try:
        ...
        lsd = await page.evaluate(r'''() => {
            try {
                // 1. window.require("LSD") — الأكثر موثوقية
                if (typeof window.require === 'function') {
                    try { const t = window.require("LSD")?.token; if(t) return t; } catch(e) {}
                }
                // 2. input/meta hidden fields
                const input = document.querySelector('input[name="lsd"]');
                if(input && input.value) return input.value;
                const meta = document.querySelector('meta[name="lsd"]');
                if(meta && meta.content) return meta.content;
                // 3. inline script — variants
                const html = document.documentElement.innerHTML;
                let m;
                m = html.match(/"LSD",\[\d+\],\{"token":"([^"]+)"\}/);
                if(m) return m[1];
                m = html.match(/"LSD",\[\d+\],\{token:"([^"]+)"\}/);
                if(m) return m[1];
                m = html.match(/"lsd":"([^"]+)"/);
                ...
                if(m) return m[1];
                // 4. script tags search
                const scripts = Array.from(document.querySelectorAll('script'));
                for(const s of scripts) {
                    const t = s.textContent;
                    m = t.match(/"LSD",\[\d+\],\{[^}]*"token":"([^"]+)"/);
                    if(m) return m[1];
                    m = t.match(/"lsd":"([^"]+)"/);
                    if(m) return m[1];
                }
...
# doc_ids مرتبة: الأول هو الأحدث، لو فشل بيجرب التالي تلقائياً
BOOST_DOC_IDS = [
    "9955578997835249",
    "7678108775563460",
    "6423087354383438",
...
async def boost_ad(request: Request):
    ...
    gender = data.get("gender", 0)        # 0=الكل، 1=ذكور، 2=إناث
    token = data.get("token", "").strip() # توكن Graph API للحصول على العملة
    ...
    act_id = extract_act_id(ad_account) if ad_account else ""

    # جلب العملة من Graph API أولاً، fallback USD
    currency = "USD"
    if token and act_id:
        try:
            r = requests.get(
                f"https://graph.facebook.com/v18.0/act_{act_id}",
    ...
    try:
        async with async_playwright() as p:
            ...
            # الخطوة 1: استخرج fb_dtsg من www.facebook.com
            ...
            # الخطوة 2: انتقل إلى business.facebook.com واستخرج lsd منها
            # هذا يحل مشكلة "Failed to fetch" (CORS من www → business)
            await page.goto('https://business.facebook.com/', wait_until='domcontentloaded', timeout=30000)
            await page.wait_for_timeout(2000)
            await inject_popup_remover(page)

            lsd = await page.evaluate(r'''() => {
                try {
                    // 1. window.require("LSD")
                    if (typeof window.require === 'function') {
                        try { const t = window.require("LSD")?.token; if(t) return t; } catch(e) {}
                    }
                    // 2. input/meta
            ...
            # استخراج fb_dtsg من business.facebook.com لو أحسن
            biz_dtsg = await page.evaluate(r'''() => {
                try {
                    if (typeof window.require === 'function') {
                        try { const t = window.require('DTSGInitialData')?.token; if(t) return t; } catch(e){}
                        try { const t = window.require('DTSGInitData')?.token; if(t) return t; } catch(e){}
                    }
                    const input = document.querySelector('input[name="fb_dtsg"]');
                    if(input && input.value) return input.value;
                    const scripts = Array.from(document.querySelectorAll('script'));
            ...
            # بناء targeting حسب الطلب الحقيقي
            targeting_dict = {
                "age_min": int(age_min) if age_min else 18,
                "age_max": int(age_max) if age_max else 65,
                "geo_locations": {"countries": countries, "location_types": ["home", "recent"]},
                "targeting_optimization": "expansion_all",
                "targeting_automation": {"advantage_audience": 0}
            }
            if gender and int(gender) > 0:
                targeting_dict["genders"] = [int(gender)]
            ...
            # جدولة تلقائية: +30 دقيقة من الآن
            import time as _time
            start_time_ts = int(_time.time()) + 1800
            ...
            creation_spec = {
                ...
                "ads_lwi_goal": "AUTOMATIC",
                "audience_option": "NCPP",
                "auto_boost_settings_id": None,
                "auto_targeting_sources": None,
            ...
            variables = {
                "input": {
                    ...
                    "target_id": f"{page_id}_{post_id}",
                    "actor_id": actor_id,
                    "client_mutation_id": "1"
                }
            }

            # FIX #4: loop على doc_ids — لو واحد فشل بيجرب التالي
            boost_json = {}
            used_doc_id = None
            for doc_id in BOOST_DOC_IDS:
                graphql_payload = {
                    "fb_dtsg": fb_dtsg,
                    "lsd": lsd or "",
                ...
                print(f"[DEBUG] doc_id={doc_id} response: {json.dumps(boost_json)[:600]}")

                errors = boost_json.get("errors") or []
                data = boost_json.get("data") or {}
                # التحقق أن الداتا فيها نتيجة فعلية وليست null كلها
                data_values = list(data.values()) if data else []
                has_real_data = any(v is not None for v in data_values)

                if errors and not has_real_data:
                    err_codes = [e.get("code") for e in errors]
                    print(f"[~] doc_id {doc_id} فشل (codes={err_codes})، بيجرب التالي...")
                    continue

                used_doc_id = doc_id
                break  # نجح أو رد جزئي نكمل منه

            # استخراج ad_id من الرد
            ad_id = None
            if "data" in boost_json and boost_json["data"]:
                if "create_boosted_component" in boost_json["data"]:
                    ...
                    if comp and isinstance(comp, dict):
                        ad_id = comp.get("id") or (comp.get("campaign") or {}).get("id")
                else:
                    for key, val in boost_json["data"].items():
                        if val and isinstance(val, dict):
                            boosted = val.get("boosted_item")
                            if boosted and isinstance(boosted, dict):
                                ad_id = boosted.get("id")
                            if not ad_id:
            ...
            # FIX #5: الـ pause عبر GraphQL internal مش Graph API رسمي
            paused = False
            if ad_id:
                pause_vars = json.dumps({"input": {"ad_id": ad_id, "status": "PAUSED"}})
                pause_payload = {
                    "fb_dtsg": fb_dtsg,
                    "variables": pause_vars,
                    "doc_id": "4215472411814298"  # AdStatusMutation
                }
                pause_result = await page.evaluate(f'''async () => {{
                    try {{
                        const res = await fetch("https://www.facebook.com/api/graphql/", {{
                            ...
                            body: new URLSearchParams({json.dumps(pause_payload)}).toString(),
                            credentials: "include"
                        }});
                        const text = await res.text();
                        try {{ return JSON.parse(text); }} catch(e) {{ return {{raw: text}}; }}
                ...
                # نعتبره paused لو مفيش error صريح في الرد
                if isinstance(pause_result, dict):
                    paused = "error" not in str(pause_result).lower() or bool(pause_result.get("data"))
                print(f"[i] Pause result: {str(pause_result)[:200]}")
            ...
            return {
                ...
                "message": "✅ تم النشر والإيقاف فوراً" if (ad_id and paused) else ("⚠️ تم النشر لكن تحقق من الإيقاف يدوياً" if ad_id else "⚠️ تم الإرسال لكن لم يتم استخراج ad_id")
            }
...
# ======================= API: Fetch Page Posts =======================
@app.post("/api/fetch_page_posts")
async def fetch_page_posts(request: Request):
    body = await request.json()
    cookies_raw = body.get("cookies", "[]")
    proxy_str = body.get("proxy", "")
    page_input = body.get("page_id", "").strip()
    token = body.get("token", "").strip()

    if not page_input:
        return {"ok": False, "reason": "أدخل معرّف أو رابط الصفحة"}

    # استخراج page_id من الرابط
    page_id = ""
    m = re.search(r'profile\.php\?id=(\d+)', page_input)
    if m:
        page_id = m.group(1)
    if not page_id:
    ...
    if not page_id:
        m = re.search(r'[?&]page_id=(\d+)', page_input)
        if m: page_id = m.group(1)
    if not page_id and re.fullmatch(r'\d+', page_input):
        page_id = page_input
    if not page_id:
        # محاولة Graph API باستخدام التوكن لاستخراج ID من اسم الصفحة
        page_name = page_input.rstrip('/').split('/')[-1].split('?')[0]
        if token and page_name:
            try:
                graph_url = f"https://graph.facebook.com/v19.0/{page_name}?fields=id&access_token={token}"
                ...
                pass
    if not page_id:
        return {"ok": False, "reason": "لم يتم استخراج معرّف الصفحة — أدخل الـ ID مباشرةً"}

    try:
        cookies = json.loads(cookies_raw) if isinstance(cookies_raw, str) else cookies_raw
    except:
        return {"ok": False, "reason": "الكوكيز غير صالحة"}
    ...
    async with async_playwright() as p:
        ...
        try:
            fb_cookies = [{"name": c["name"], "value": c["value"], "domain": ".facebook.com", "path": "/"} for c in cookies if c.get("name") and c.get("value")]
            await context.add_cookies(fb_cookies)
        except Exception as e:
            await browser.close()
            return {"ok": False, "reason": f"خطأ كوكيز: {e}"}
        ...
        try:
            ...
            if not fb_dtsg:
                await browser.close()
                return {"ok": False, "reason": "لم يتم استخراج fb_dtsg — تحقق من الكوكيز"}

            variables = {
                "callerID": "BIZWEB_PUBLISHED_POSTS",
                "count": 25,
                "filters": [{"param": "UNIFIED_TABLE_FEATURE", "values": []}],
                "ids": [page_id],
                "initialFilters": [{"param": "UNIFIED_TABLE_FEATURE", "values": ["LOAD_ONLY_VISIBLE_COLUMNS_IN_VIEWPORT"]}],
                "scale": 1,
                "searchString": "",
                "sortDirection": "DESC",
                "sortKey": "DATE",
                "timeRange": {"type": "LIFETIME"},
                "visibleColumnKeys": ["TITLE", "DATE", "REACH", "NET_INTERACTIONS"]
            }
            payload = {
                "fb_dtsg": fb_dtsg,
                "lsd": lsd or "",
                "variables": json.dumps(variables),
                "doc_id": "27589038100735926"
            }
            ...
            result = await page.evaluate(f'''async () => {{
                try {{
                    const res = await fetch("https://business.facebook.com/api/graphql/", {{
                        ...
                        headers: {{
                            "Content-Type": "application/x-www-form-urlencoded",
                            "X-FB-Friendly-Name": "useBizWebUnifiedTableInitialLoad_data_refetchable",
                            "X-FB-LSD": "{lsd or ''}"
                        }},
                    ...
                    const text = await res.text();
                    try {{ return JSON.parse(text); }} catch(e) {{ return {{raw: text.substring(0, 2000)}}; }}
                }} catch(err) {{ return {{error: err.message}}; }}
            }}''')

            print(f"[DEBUG fetch_posts] page_id={page_id} response: {json.dumps(result)[:800]}")

            # تحليل الرد واستخراج المنشورات
            posts = []
            def deep_find_edges(obj, depth=0):
                if depth > 8 or not isinstance(obj, (dict, list)):
                    return
                if isinstance(obj, list):
                    for item in obj:
                        deep_find_edges(item, depth+1)
                    ...
                    if "edges" in obj and isinstance(obj["edges"], list):
                        for edge in obj["edges"]:
                            ...
                            title = None
                            date_val = None
                            # محاولة استخراج entity_id / id
                            post_id = node.get("entity_id") or node.get("id") or node.get("node_id")
                            # من columns_data
                            cols = node.get("columns_data") or {}
                            if isinstance(cols, dict):
                                t_col = cols.get("TITLE") or {}
                                if isinstance(t_col, dict):
                                    title = t_col.get("text") or t_col.get("value")
                                d_col = cols.get("DATE") or {}
                                if isinstance(d_col, dict):
                                    date_val = d_col.get("value") or d_col.get("timestamp")
                            if not title:
                                title = node.get("title") or node.get("text") or node.get("message")
                            if post_id:
                                # إزالة page_id_ prefix لو موجود
                                pid_str = str(post_id)
                                if "_" in pid_str:
                                    pid_str = pid_str.split("_")[-1]
                                if not any(p["post_id"] == pid_str for p in posts):
                                    posts.append({
                                        "post_id": pid_str,
                                        "story_id": f"{page_id}_{pid_str}",
                                        "title": (str(title)[:80] if title else f"منشور {pid_str}"),
                                        "date": date_val
                                    })
                    for v in obj.values():
                        deep_find_edges(v, depth+1)

            if isinstance(result, dict) and "data" in result:
                deep_find_edges(result["data"])

            await browser.close()
            if not posts:
                return {"ok": False, "reason": "لم يتم العثور على منشورات", "raw": json.dumps(result)[:400]}
            return {"ok": True, "posts": posts, "page_id": page_id}

        except Exception as e:
            await browser.close()
            return {"ok": False, "reason": f"خطأ: {str(e)[:200]}"}
