...
function ToolCard({ emoji, title, badge, children, comingSoon }) {
  return (
    <div style={{
      background: C.panel,
      border: `1px solid ${comingSoon ? C.border : 'rgba(245,158,11,0.25)'}`,
      borderRadius: 14,
      padding: '16px',
      display: 'flex',
      ...
      {comingSoon && (
        <div style={{
          ...
          <div style={{
            ...
            color:C.gold, fontSize:11, fontWeight:700, letterSpacing:'0.12em',
...
function ExtensionInstaller() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      {/* Description */}
      <div style={{ padding:'10px 12px', background:'rgba(34,197,94,0.07)', border:'1px solid rgba(34,197,94,0.2)', borderRadius:10, fontSize:11, color:C.textSub, lineHeight:1.8, fontFamily:"'Tajawal',sans-serif" }} dir="rtl">
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:6 }}>
        {[
          ...
          <div key={f.label} style={{ padding:'6px 10px', background:'rgba(255,255,255,0.04)', border:`1px solid ${C.border}`, borderRadius:8, fontSize:10, color:C.textSub, display:'flex', alignItems:'center', gap:6 }}>
      ...
      {/* Steps */}
      <div style={{ fontSize:11, color:C.textMuted, fontWeight:700, letterSpacing:'0.06em', marginBottom:2, fontFamily:"'Share Tech Mono',monospace" }}>◈ خطوات التثبيت</div>
        ...
        {steps.map((s, i) => (
          <div key={i} onClick={() => setStep(i)}
            ...
            <span style={{
              ...
              display:'flex', alignItems:'center', justifyContent:'center',
              fontSize:9, fontWeight:800, fontFamily:"'Share Tech Mono',monospace",
            }}>{s.num}</span>
            <div>
              <div style={{ fontSize:11, fontWeight:700, color: step === i ? C.gold : C.text }}>{s.title}</div>
              <div style={{ fontSize:10, color:C.textMuted, lineHeight:1.6, marginTop:1 }} dir="rtl">{s.desc}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ fontSize:10, color:C.textMuted, textAlign:'center', fontFamily:"'Share Tech Mono',monospace" }}>
    ...
    <div style={{ display:'flex', height:'100%', width:'100%', flexDirection:'column', background:C.bg, fontFamily:"'Tajawal','Segoe UI',sans-serif" }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10, color:C.textMuted }}>مجموعة أدوات Meta المتكاملة — مجانى مؤقتاً</div>
            </div>
          </div>
          {/* FREE badge */}
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2 }}>
            ...
            <div style={{ fontSize:9, color:C.textMuted, fontFamily:"'Share Tech Mono',monospace" }}>FREE · TEMP</div>
          </div>
        </div>
      </header>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'16px 20px', display:'grid', gridTemplateColumns:'220px 1fr', gap:14 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          <div style={{ fontSize:9, color:C.textMuted, fontFamily:"'Share Tech Mono',monospace", letterSpacing:'0.08em', marginBottom:4 }}>◈ 8 TOOLS</div>
          {tools.map((tool, i) => (
            <button key={tool.key} onClick={() => setSelected(i)}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:11, fontWeight:700, color: selected===i ? C.gold : C.text, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{tool.title}</div>
                {tool.badge ? (
                  <span style={{ fontSize:9, color:C.green, fontFamily:"'Share Tech Mono',monospace" }}>● {tool.badge}</span>
                ) : (
                  <span style={{ fontSize:9, color:C.textMuted, fontFamily:"'Share Tech Mono',monospace" }}>⏳ قريباً</span>
                )}
              </div>
            </button>
          ))}
          ...
          {selected === 0 ? (
            ...
            >
              ...
              <div style={{ padding:'10px 12px', background:'rgba(245,158,11,0.06)', border:'1px solid rgba(245,158,11,0.2)', borderRadius:8, textAlign:'center' }}>
                <div style={{ fontSize:11, color:C.gold, fontWeight:700 }}>🚧 قيد التطوير</div>
                <div style={{ fontSize:10, color:C.textMuted, marginTop:4 }}>سيتم إضافة هذه الأداة قريباً ضمن مجموعة Vortex Meta Tools</div>
              </div>
            </ToolCard>
          )}
        </div>
