 ...
 * because Meta has no public API for this action.
 */
...
async function launchBrowser(proxyConfig) {
  ...
  // Try playwright-extra + stealth first
  try {
    const pExtra = require('playwright-extra');
    const stealth = require('puppeteer-extra-plugin-stealth');
    pExtra.chromium.use(stealth());
    return await pExtra.chromium.launch(opts);
  } catch (_) {
...
router.post('/add-cards', async (req, res) => {
  ...
  if (!parsed.length) {
    return res.json({ ok: false, reason: 'لا يمكن تحليل أي بطاقة', parseErrors });
  }
  ...
  // Run with concurrency (cap at 3 to avoid resource exhaustion)
